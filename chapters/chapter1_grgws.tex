...
This section contains the discussion on the linearising the Einstein's field equation and the solutions of the linearised field equation. We discuss the solution in vacuum and far away from the source. I follows the treatment from commonly used textbooks on general relativity and gravitational waves~\cite{Misner:1973prb, Maggiore:2007ulw}
...
\end{equation} 
where we need to ensure that $\left|\partial_{\nu}\xi_{\mu}\right|\lesssim\left|h_{\mu\nu}\right|$ so that perturbations remain small. Under the transformations proposed in Eq.~\eqref{eq:transform_vector}, the trace-reversed metric perturbations transform as
\begin{equation}
    \label{eq:transform_metric}
    \bar{h}_{\mu \nu} \rightarrow \bar{h}^{\prime}_{\mu \nu} =  \bar{h}_{\mu \nu}-\left(\partial_\mu \xi_\nu+\partial_\nu \xi_\mu-\eta_{\mu \nu} \partial_\sigma \xi^\sigma\right) .
\end{equation}
Such transformations do not change the value of the tensor components, a solution of the linearised field equations remains a solution in every coordinate systems obtained under these transformation. Therefore, when choosing a coordinate system, we have the freedom to choose one that can simplify Eq.~\eqref{eq:g_hbar}. Specifically, we can always find a transformation under which
\begin{equation}
\partial^{\nu}\bar{h}^{\prime}_{\mu\nu} = 0.
\end{equation}
This choice is called the harmonic gauge. The $\xi^{\nu}$ for such a transformation is obtained by solving the follwing equation
\begin{equation}
    \square\xi_{\nu} = \partial^{\mu}\bar{h}_{\mu\nu},
\end{equation}
using an appropriate Green's function. The harmonic gauge leaves room for additional transformations which satisfy the equation, $\square\xi^{\nu} = 0$. I explain the consequence of it later. In harmonic gauge, the linearised field equation \eqref{eq:g_hbar} further simplifies to
\begin{equation}
\label{eq:liner_last}
...
\end{pmatrix}
\end{equation}
for a gravitational wave propagating along $z$-direction. The names follow from the phenomena that if a gravitational wave passes by ring of beads in vacuum, the plus (cross) polarization will periodically stretch and squeeze the ring in $+~(\times)$ shape. 
...
Here, we can draw analogy from the calculation of electromagnetic radiation from arbitrary charge distribution at a distance. This calculation is usually done using multipole expansion. Each term in the series obtained from multipole expansion approximates the spatial distribution of the source and represents the contribution to far-field radiation in decreasing order. The first term (also known as the monopole term) in the expansion is simply the total charge which does not contribute to the electromagnetic radiation due to law of conservation of electric charge. Therefore, the dominant contribution to the electromagnetic radiation is due to the dipole term.  
...
\end{equation} 
where $\hat{n}$ is the unit vector along $|\vec{x}-\vec{y}|$. Approximating the reciprocal term $\frac{1}{|\vec{x}-\vec{y}|} \approx \frac{1}{r}$ and expanding the the stress-energy tensor around $t-r$,
\begin{equation}
...
\end{equation}
where $M^{kl} = \int \mathrm{d}\vec{y} T^{00} y^ky^l$ is the mass quadrupole moment. It shows the leading order contribution to the far-field gravitational radiation. The mass and linear momentum is conserved only in the case of the linearised theory. A radiating body in fact loses mass and linear momentum. However, the absence of monopole and dipole term holds more generally. It can be derived in the full non-linear treatment of the field equation. 
...
A schematic diagram of an L-shaped interferometer is shown in Figure~\ref{fig:det_l}. A beam of laser that is split into two travels through the two arms which are of equal length. The light bounces back from the mirror at the end of the arm. The laser travels equal distance in each arm in the absence of an external disturbances, e.g. absence of a gravitational wave. When a gravitational wave passes by, it periodically stretches and squeezes the arms so the light no longer travels the same distance in both arms. By observing the constructive and destructive interference pattern in the light collect by the photo diode, we can measure the strain $(h(t))$ of the gravitational wave signal. The strain is equal to the relative change in arm length $(L)$
\begin{equation}
    h(t) =  \frac{\Delta L}{L},
\end{equation}
...
Here, $F_+$ and $F_{\times}$ are called antenna pattern functions of the detector. They enable the projection of the GW polarizations from source frame to the detector frame. Unlike the conventional electromagnetic telescopes, the GW detectors are not pointed in any certain direction. Their orientation changes only with the rotation of Earth. Their sensitivity around the sky indeed depends on the sky-location, polarization angle of the gravitational wave, and the arrival time of the signal at the detector. Each $F_i$ is a function of four parameters
\begin{equation}
    F_i := F_i(\alpha, \delta, \psi, t_c),
\end{equation}
where $\alpha$ and $\delta$ denote the right ascension and declination, respectively. The polarization angle is denoted by $\psi$ and time of arrival by $t_c$. Their sensitivity is periodic over the duration of $\sim$24 hours. The index $i$ represents the two polarizations.
