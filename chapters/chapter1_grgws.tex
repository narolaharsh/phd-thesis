...
\begin{itemize}
    ...
    \item{GR and GWs}
    \item{Figure and table to show as many binary parameters possible}
\end{itemize}
...
To solve the linearised field equation far away from the source (which where our GW detectors are), we can safely assume that the stress-energy is equal to zero. Therefore, Eq.~\eqref{lineq} reduces to 
\begin{equation}
    \label{eq:waveq}
    \Box \bar{h}_{\mu\nu} = 0.
\end{equation}
Eq.~\eqref{eq:waveq} is known as the wave equation as it admits plane-wave solutions, 
\begin{align}
    \bar{h} &= A_{\mu\nu} \cos{\left(k_{\alpha}x^{\alpha}\right)},\\
    k_{\mu}^{\mu} &= 0.
\end{align}
which are the perturbation of a flat spacetime far away from the source that produced them. 

\subsubsection{Plus and cross polarization}
Due to Lorentz gauge, 
\begin{equation}
    k^{\mu}A_{\mu\nu} = 0, 
\end{equation}
