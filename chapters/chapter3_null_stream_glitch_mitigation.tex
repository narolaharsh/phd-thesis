...
The null stream provides us with an extra piece of information in the form of Eq.~\ref{eq:null-stream-eq}. For the first time, we show how this information can be used to address a pressing problem posed by glitches. We highlight the key features of \texttt{nijntje} framework and \ETT configuration in table \ref{tab:nijntje_conclusion}. 
...
\begin{table}[h!]
    ...
    \midrule
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        \item Biases expected in the parameter estimation execpt for the edge cases when glitch and signals are far apart
        \item True signal model needs to be known \textit{apriori}. Biases worsen if used without using a signal model / using incorrect signal model. 
        \item $\mathcal{O}(10)$ hours per analysis
        \item Adaptations required depending on the type of the GW signal and glitch. 
        \item Biases worsen when two interferometers have unequal SNRs. 
    \end{enumerate}
    &
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        \item Restore the accuracy of parameter estimation which is similar to the case when no glitch introduced
        \item Does not require any prior knowledge of the signal model. Ready to use even when the signals are expensive to simulate or challening to model. 
        \item $\mathcal{O}(1)$ hour per analysis.
        \item Ready to use against varying morphologies of glitches.
        \item Individual detector SNR does not play any role. 
    \end{enumerate}
    \\
    \bottomrule
    \end{tabular}
    \caption{Addressing the problem of signal-overlapping glitches with \ETT and ET-2L}
