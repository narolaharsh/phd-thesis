...
We consider a typical scenario where a glitch is overlapping with a GW signal. While the glitches exhibit a range of morphology, here we choose a morphology that is ubiquitous in the data, i.e. the blips. For the sake of the example, we assume that the only one interferometer encounters the glitch. The rest of the two interferometers of the triangle contains a GW signal in Gaussian noise. The GW source is placed at redshift of 2, where the merger rate distribution of BBH peaks, making it a typical source. The source frame masses of the two black holes are $38\,M_\odot$ and $33\,M_\odot$. This choice of parameters results into a GW signal with network signal-to-noise ratio (SNR) of 83 divided equally among three interferometers. The starting frequency of our analysis is 20 Hz, sampling frequency is 2048 Hz, and we use ET-D PSD to simulate Gaussian noise. We inject a blip glitch in ET$_1$ interferometer such that is overlapping with GW signal. We ensure that the SNR of the glitch is similar to the SNR of the GW signal in ET$_1$, i.e. $83/\sqrt{3} \sim 47$. Thanks to recent developments in machine learning based methods, it is now possible to simulate glitches.  
...
Before we present the parameter estimation results, we showcase the results of glitch mitigation, i.e., how close the reconstructed glitch is to the true glitch. This serves as a precursor to the parameter estimation results since accurate glitch mitigation leads to the accurate parameter measurement. A visualization of the glitch reconstruction is presented in Figure~\ref{fig:glitch_overlap}. The ET triangle configuration, due to the null stream, achieves a more accurate glitch reconstruction by avoiding mismodeling from the GW signal. In contrast, for the ET-2L configuration inevitably some additional features from the signal are incurred. 
...
Next, we turn to the results of parameter estimation when glitch precisely conincides with the GW merger. Figure~\ref{fig:delta_zero} presents a comparison of the posterior distributions of the GW signal parameters after the glitch is mitigated, for $\Delta t = 0$. For all parameters shown, glitch mitigation with null stream for \ETT~results in posteriors 
...
Finally, we turn to the scenarios when glitch is not coninciding with the merger. Figure~\ref{fig:trend} shows that the characteristics of extrinsic parameter recovery remain consistent 
...
are wider or generally exclude the true values, except when the glitch onset is 100 ms before or 20 ms after the merger. We infer that at those values of $\Delta t$, the glitch is far enough from GW signal to cause any biases. 

\section{Conclusions and future work}
The null stream provides us with an extra piece of information in the form of Eq.~\ref{eq:null-stream-eq}. For the first time, we quantitatively show how this information can be used to address the problem posed by glitches. Further, in the table \ref{tab:nijntje_conclusion}, we present a qualitative comparison between the \ETT and ET-2L~configuration in context of glitch mitigation. 
...
\begin{table}[h!]
    ...
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        ...
        \item True signal model needs to be known \textit{apriori}. Biases worsen if signal model is not known or incorrect.
        \item $\mathcal{O}(10)$ hours per analysis. Difficult to scale to meet the demands of 3G-era. \textcolor{white}{lorem ipsum lorem ipsum.}
        \item Modification required depending on the type of the GW signal and glitch. Modification required when multiple signals and glitches are overlapping with eachother -- a scenario highly likely in the 3G-era.
        \item Biases worsen when two interferometers have unequal SNRs.
    \end{enumerate}
    &
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        \item Quality of measurements identical to the scenario when glitches are not introduced.
        \item Does not require any prior knowledge of the signal model. Ready-to-use for all kinds of signals. 
        \item $\mathcal{O}(1)$ hour per analysis. Fast and accurate framework \texttt{nijntje}; a ready-to-use tool for the 3G-era. 
...
We plan to extend this work by including various kinds of signals and glitches in our analysis. We also plan to investigate the possibility of occurance and mitigation of coincident glitches, i.e., glitches that appear in all interferometers of a network at the same time. We also plan to investigate the impact of newtonian noise -- the models of which are poorly known at the time of writing of this thesis. In addition, the null stream can be constructed when all three interferometers are online. We leave it to the future work to include the duty cycle in our analysis. Note that, in the absence of null stream, one can revert to performing joint signal and glitch mitigation for the triangular configuration.

The problem posed by glitches is a pressing one and it could hinder our transition to the precision science era. The comparison presented in this chapter shows a clear edge the null stream inherent in the triangular configuration has to offer, over the alternate configuration. 
