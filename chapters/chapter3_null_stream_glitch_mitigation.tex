...
Before we carry out such a comparison, we need to develop certain building blocks which are missing at the time of writing this thesis. Specifically, our knowledge about glitches, i.e., their morphologies, rates, and SNR distribution, is largely based on the glitches of LIGO and Virgo interferometers. It is possible that the glitches in the 3G era do not follow similar distributions. The low frequency regime (5 to 20 Hz) and the high frequency regime (above $\approx 10^3$ Hz) are of special interest in the 3G era. However, the distribution of glitches in these regimes is relatively unknown and may be difficult to determine until the 3G detectors become operational. The frequency regimes are currently outside of typical LIGO-Virgo bandwidths. Additionally, the codebase to perform glitch mitigation for the 2L configuration, i.e. BayesWave, is tuned to analyse the LIGO-Virgo data. Though in our work we have made the best use of this state-of-the-art framework, a next-generation of BayesWave or a similar tool is required to perform the comparison between 2L and triangular configuration of the Einstein Telescope Specifically, a tool that can remove glitches from the long, loud, and overlapping signals for the 2L configuration is needed. 
