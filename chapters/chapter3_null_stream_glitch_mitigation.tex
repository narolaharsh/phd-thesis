...
Fig.~\ref{fig:delta_zero} presents a comparison of the posterior distributions of the GW signal parameters after the glitch is mitigated, for $\Delta t = 0$. For all parameters shown, glitch mitigation with null stream for \ETT~results in posteriors 
...
\begin{figure}
    \centering
    \includegraphics[width=.75\textwidth]{../figures/newsnr_rank_10_single_block_percentile_parameters_short.pdf}
    ...
    \label{fig:trend}
\end{figure}
Fig.~\ref{fig:trend} shows that the characteristics of extrinsic parameter recovery remain consistent 
...
The null stream provides us with an extra piece of information~\ref{eq:null-stream-eq}. For the first time we show how we can use this information for precision science. 
