...
\begin{table}[h!]
    ...
    \midrule
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        \item Able to undo the effect of the glitch for all scenarios and restore the accuracy of the parameter estimation.
        \item Does not require any prior knowledge of the signal model. Ready to analyze scenarios when the signals are expensive to simulate or challening to model. 
        \item $\mathcal{O}(1)$ hour per analysis.
        \item Robust against varying morphologies of glitches.
    \end{enumerate}
    &
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        \item Biases expected in the parameter estimation execpt for the edge cases when glitch and signals are far apart
        \item Signal model needs to be known aprior. Biases worsen if used without the correct signal model. 
        \item $\mathcal{O}(10)$ hours per analysis
        \item Needs to be adapted depending on the type of the GW signal and glitch. 
    \end{enumerate}
    \\
    \bottomrule
    \end{tabular}
