...
When performing unmodelled reconstruction, the dimensionality of the parameter space of $\vec{\theta}$ is not known unlike the modelled reconstruction explained in chapter~\ref{ch:gwpe}. While the Bayesian framework, GW likelihood, and Bayesian approximation still remains robust, the Nested sampling algorithm from previous chapter is not a suitable choice to explore such a parameter space. Here, we review a transdimensional algorithm that is routienly used to perform unmodelled reconstruction. 
...
Morlet-Gabor wavelets are sutible for the problem at hand because glitches $(\vec{g})$ we want to model can be expressed as linear combination of $N$ wavelets [BayesWave first paper],
\begin{equation}
    \vec{g}(t; \vec{\theta}) = \sum_{k = 0}^{N} \vec{\psi}_k(t; \vec{\theta}_k),
\end{equation}
where $\vec{\theta}$ is set of $N\times5$ parameters since each $\vec{\psi}_k$ needs 5 model parameters shown in Eq.~\eqref{eq:wavelet}. 
Anoter desirable feature is that Morlet-Gabor wavelets preserve shape when switching between time-frequency domains and they have a simple analytic expression in both domains. 

Here, $N$ itself is not a fixed parameters which means the parameters space we want to explore also does not have a fixed size. Therefore, we need to employ a sampling algorithm that is capable of sampling over a variable dimensional parameter space. 
