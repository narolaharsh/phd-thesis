...
Let us consider the scenarior when we are jumping from a one wavelet state $\vec{\theta}$ to a two wavelete state $(\vec{\theta'})$. Note that $\vec{\theta} (\vec{\theta'}s)$ has five (ten) dimensions. Since the current state has fewer dimensions then the proposed state, we need an auxiliary probability distribution and a mapping function to account for the missing dimensions. These are user-made choices, typically made by trial and error. 

The user may choose any auxiliary probability distribution from which it is easy to sample as long as it makes up for the missing dimensions. Here, we choose a five dimensional uniform distribution for the sake of the argument. The user may choose any mapping function but it must be bijective and continuously differentiable i.e. a diffeomorphism. Let the bijective function be an exponential so the logarithmic function will be its inverse. 

For simplicity of the notation, we write down only the wavelet amplitude parameters (see Eq.~\eqref{eq:wavelet}) of current state ($A_1$) and the proposed state $(A'_1, A'_2)$. Let $w$ be a sample from $p_{\mathrm{aux}}$

\begin{equation}
    (A_1, w) \xrightarrow[]{f} (A_1, e^{w}) \coloneq (A'_1, A'_2)  
\end{equation}
Hence, $f$ maps $A_1$ to $A'_1$ and $w$ to $e^w$. Now, the dimension of the current state matches with the dimensions of the proposed state. Another remaining factor is the determinant of the Jacobian
...
\subsubsection{From higher to higher dimension}
When going from high dimensions to low dimensions, it is incorrect to simply do
\begin{equation}
   \mathrm{Incorrect:} ~~ ~ ~ ~ ~ (A'_1, A'_2) \rightarrow (A_1)
\end{equation}
One needs to define the inverse of $f$, which is the logarithmic function since we chose $f$ to be the exponential function. Let us denote the inverse by $f'$, then when going from high dimension to low dimensions, 
\begin{equation}
    (A'_1, w') \xrightarrow[]{f'} (A_1, \ln{w'})
\end{equation}
where $w'$ is either chosen from another auxiliary distribution $p'_{\mathrm{aux}}$ and it may or may not be same as the earlier auxiliary distribution $p_{\mathrm{aux}}$. The Jacobian in this case  will turn out to be
...
\begin{equation}
    \alpha = \mathrm{min} \left \{1,~\frac{p(\vec{\theta}  | \vec{d})}{p(\vec{\theta'} | \vec{d})}~p_{\mathrm{aux}}(w')~\frac{1}{w'}  \right \}
\end{equation}
