...
\begin{figure}
    \centering
    \includegraphics[width=12cm]{../figures/gw170817scan.pdf}
    \caption{A loud glitch overlapping with the merger of the binary neutron star signal GW170817 in LIGO-Livingstone interferometer. Dotted white arrows are added to help trace the signal.}
    \label{fig:bns_scan}
\end{figure}
A case where theoretical models do not exist can be made for instrumental noise transients or \textit{glitches}. While the background noise in the GW detectors is assumed to be Gaussian and stationary, occurance of a glitch violates this assumption. As most of our physics inferences assume that noise in the detector is Gaussian-stationry, a violation of it could corrupt the physics inferences. Depending on the time of occurance, glitches can be problematc in two ways,
\begin{itemize}
\item Glitches occuring near a GW signal could corrupt the measurements of its source parameters. Therefore, it becomes necessary to carefully subtract the glitch without the contaminating the signal, \textit{before} we measure the source parameters. [GW170817 figure]
\item Isolated glitches, the ones not overlapping with a GW signal, increase the false alarm rate of GW detections since some glitches look similar to a GW signal. For example, the blip glitches have similar time-frequency evolution as GW signals from high-mass BBH mergers [Figure]. It becomes important to identify and veto glitchy data from the analysis to reduce false alarms.
\end{itemize} 
Glitches could be caused by instrumental (control systems, scattered light) or environmental (earthquakes, wind, anthropogenic) factors, though their origins often remain unknown in many cases [Figure Figure Figure]. Thefore, it is not possible to devise a theoretical model for glitches in most cases and we need to rely on the unmodelled approach to reconstruct and subtract glitches. 
