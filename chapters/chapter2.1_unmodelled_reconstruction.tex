...
Metropolis-Hastings algorithm makes use of a proposal density function ($\Gamma$) to determine whether a proposal point $\vec{\theta}'$ should be accepted. The probability that a proposal $\vec{\theta}'$ is accepted is then given by $\alpha$
\begin{equation}
    \label{eq:mh_algo}
...
\end{itemize}
The steps of performing a stationary move are trivial and same as Metropolis-Hastings algorithm. The birth and death moves are typically the most complicated ones. Below I explain how to make those moves.  
...
\begin{align}
    \alpha_r &= \frac{p(\vec{\theta}', D'| \vec{d})}{p(\vec{\theta}, D | \vec{d})} \frac{\Gamma(\vec{\theta}, D | \vec{\theta}', D')}{\Gamma(\vec{\theta}', D' | \vec{\theta}, D)} \\
...
\end{equation}
Let's say both, one and two wavelet, states are equally likely, then $\Gamma(D | D') = \Gamma(D' | D) = 0.5$. If we were dealing with N different states and if each states were equally likely, then the transition probability between any two states will be $1/N$. 
...
where $\vec{\theta}'$ is the proposal under consideration with $D'$ dimensions, $\vec{\theta}$ is the previously accepted point with $D$ dimensions, $\Gamma$ is the proposal density function, and $\vec{d}$ is data. For the sake of argument, let consider that $\theta$ represents a state of 1 wavelet and $\theta'$.
