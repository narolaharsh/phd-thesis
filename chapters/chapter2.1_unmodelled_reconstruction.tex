...
A case where theoretical models do not exist can be made for instrumental \textit{glitches}. While the background noise in the GW detectors is assumed to be Gaussian and stationary, they often encounter noise transients, also known as glitches, that violate the assumption. Depending on the time of occurance, glitches can be problematc in two ways 
\begin{itemize}
\item Glitches occuring near a GW signal could corrupt physics inferences made from the data. In order to make measurements, it becomes necessary to carefully subtract the glitch without the contaminating the signal, \textit{before} we make physics inferences.
\item Isolated glitches, the ones not overlapping with a GW signal, increase the false alarm rate of GW detections as some glitches look similar to a GW signal. For example, the blip glitches have similar time-frequency evolution as GW signals from high-mass BBH mergers. It becomes important to identify and veto glitchy data from the analysis to reduce false alarms.
\end{itemize} 
