...
\begin{itemize}
    \item The rate of convergence depends on the choice of auxiliary distribution and bijective functions. A universal choice that works for all problems do not exist. 
    \item The termination criteria is not well defined.
    \item In contarst to Nested sampling, the aim here is to obtain the posterior probability distribution. Additional computations are needed to obtain evidence. 
    \item The wavelets are not orthogonal
    \item Proposed to select two competing dictribution and an overkill. 

\end{itemize}
\section{Problems of glitches in LIGO-Virgo and third-generation detectors}
...
\begin{table}[h!]
    ...
    \begin{enumerate}[leftmargin=*, label=\arabic*.]
        ...
        \item Very few glitches can be vetoed without losing the underlying GW signals. 
    \end{enumerate}
    \\
    \bottomrule
    \end{tabular}
...
\subsubsection{Signal overlapping glitches}
