...
\end{equation}
where $\vec{h}$ denotes the GW signal model computed for the source paraemters $\vec{\theta}$. The brackets $\left<.|.\right>$ denote the noise weighted inner product 
\begin{equation}
    \left<\vec{a}|\vec{b}\right> = \frac{4}{T} \sum_{f} \frac{\tilde{a}(f)\tilde{b}^{*}(f)}{S_{n}(f)},
\end{equation}
