...
where the integrand on the right hand side is simply the product of the likelihood and the prior defined in Eq.~(\ref{eq:bayes}), i.e., $Z$ is the normalization constant from that equation.
...
$$
X(\lambda) = \int_{\vec{\theta}: \{\mathcal{L}(\vec{\theta}) \ge \lambda}\}~\pi(\vec{\theta})~\mathrm{d}\vec{\theta},
$$
which is the prior mass obtained by integrating $\pi(\vec{\theta})$ over the region of parameter space where $\mathcal{L}(\vec{\theta}) > \lambda$. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
by $\mathcal{L}(\vec{\theta})$ and perform following change of variables,
\begin{equation}
\mathrm{d} X = \pi(\vec{\theta})~\mathrm{d}\vec{\theta}
\end{equation}
where $\int \mathrm{d} X = 1$ since the prior is normalized to 1. Substituting it in Eq~\ref{eq:evidence}, we get
$$
Z = \int_0^{1} \mathrm{d} X L(X)
$$
