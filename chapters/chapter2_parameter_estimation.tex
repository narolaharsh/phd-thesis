...
\section{Sampling}
When performing parameter estimation, we need to evaluate the likelihood function on large parameter space. For the sake of the argument, if we assume that we choose 10 points along each dimension of the parameter space, we need to perform $10^15$ to $10^17$ likelihood evaluation where each evaluation takes $\mathcal{O}(10)$ milliseconds. It must also be noted that 10 points may be too sparse to cover each dimensions. Therefore, we rely on various sampling techniques to explore the likelihood function on large parameter space. 
