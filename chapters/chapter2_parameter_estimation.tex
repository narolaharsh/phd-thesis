...
\subsubsection{Toy model for nested sampling}
I will write it if time permits
...
\begin{equation}
    p_T(\vec{\theta} | \vec{d}) = p(\vec{d} | \vec{\theta})^{1/T} \pi(\vec{\theta}).
\end{equation}
When $T=1$, we obtain the posterior and when $T=\infinity$, we obtain the prior. Defining the evidence as a function of $\beta \equiv 1/T$,
\begin{equation}
    Z(\beta) = p(\vec{d} | \beta) = \int \mathrm{d} \vec{\theta}~p(\vec{d} | \vec{\theta})^{\beta}~\pi(\vec{\theta}),
\end{equation}
where $\beta=1$ is the evidence and $\beta=0$ gives the normalized prior volume.
\begin{align}
    \ln{p(\vec{d})} &= \ln{p(d | \beta = 1)} \\
    &= \int_0^{1} \frac{d}{d\beta}\ln{p(\vec{d} | \beta)} + \ln{p(\vec{d} | \beta = 0)} \\
    & = 
\end{align}

Taking the derivative w.r.t. $\beta$ after operating operating with log on both sides,
\begin{equation}
    \frac{\mathrm{d}}{\mathrm{d}\beta} p(\vec{d} | \beta) = \left< \ln{p(\vec{d} | \vec{\theta})}\right>_{\beta}
\end{equation}
where $\left<.\right>_{\beta}$ is the expectation value of the quantity at $\beta$.

The Metropolis-Hastings algorithm Indeed, the efficieny of this algorithm depends on the choice of $Q$. In addition, each chain is started at random point in the parameter space and it requires certain number of iterations, also known as \textit{burn-in} iterations. before the dependence on the starting point is lost. we Figure~\ref{fig:mcmc} shows an e

\subsubsection{Reversible Jump}

\subsection{Comparison between MCMC and Nested Sampling}
