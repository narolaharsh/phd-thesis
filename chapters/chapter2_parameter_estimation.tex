...
Since the aim of the Nested sampling algorithm is to compute the evidence Z in small increament of $\Delta Z$, we may choose to terminate when the increament is smaller then a  desired number $\epsilon$,
\begin{equation}
    \ln{\left(Z_i + X_i \mathcal{L}_{\mathrm{max}}\right)} - \ln{(Z_i)} < \epsilon,
\end{equation}
where $\mathcal{L}_{\mathrm{max}}$ is the maximum values of likelihood encountered during sampling. 
...
\begin{equation}
p_t(\vec{\theta}) = \int \mathrm{d}\vec{\theta}'~T(\vec{\theta}, \vec{\theta}')~\vec{\theta}'
\end{equation}
where $\vec{\theta}$ is the proposed point and $\vec{\theta}'$ point from previous iteration and $T$ is the transition probability. 

\subsubsection{How are the posteriors samples and evidence estimated}
