...
In theories of gravity with large extra dimensions, there is the possibility of some energy of the GWs leaking into them \cite{Dvali:2000hr, Calcagni:2019kzo, LIGOScientific:2018dkp}, while EM radiation is confined to the usual three spatial dimensions. This would make the detected signal appear weaker, leading to larger measured values for $\DGW$ than would otherwise be the case. For definiteness, we will work with the following simple phenomenological ansatz for the relation between $\DGW$ and $\DEM$, based on conservation of integrated flux \cite{Pardo:2018ipy}:
\begin{equation}
    \label{eqn:D-model}
 \frac{\DGW}{\mathrm{Mpc}} = \left(\frac{\DEM(z_s, H_0)}{\mathrm{Mpc}}\right)^{\frac{D-2}{2}},
\end{equation}
where $D$ is the number of spacetime dimensions and $z_s$ is the source redshift~\footnote{Cardoso \textit{et al.}~\cite{Cardoso:2002pa} arrive at a similar scaling between $\DGW$ and $\DEM$ as~Eq.~\eqref{eqn:D-model} for a $D$-dimensional space time, apart from a frequency dependent factor. The frequency dependent factor determines the length scale of the effect based on the frequency of the GW signal whereas~Eq.~\eqref{eqn:D-model} presents a simplified ansatz, i.e., without a frequency dependence, following~\cite{Pardo:2018ipy}.}. We will allow $D$ to be a real number, with the GR value $D = 4$ as a fiducial value. An illustration of the effect of extra dimensions on a GW signal is given in the left plot of Figure~\ref{fig:wf-effects}.
