...
\end{equation}
Note, that the $\vec{d}$ is now replaced by $\vec{d}_1$ and $\vec{d}_2$ at relevant places. Next we construct the RBGrid only for image 1. Compute image 2 waveform using the Eq.~\eqref{wf-mapping}. Use RBGrid to generate the summary data and using the summary data, we can approximate the likelihood. 
