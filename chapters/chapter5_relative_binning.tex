Bayesian inference discussed in chapter~\ref{ch:gwpe} is the key to achieving a wide range of scientific insights from gravitational wave signal. The cost of this analysis increases with the complexity of the signal model and the signal's duration. With standard methods, such analyses require at least a few days. Moreover, for third generation detectors, due to the lowered minimum sensitive frequency, the signal duration increases, leading to even longer analysis times. With the increased detection rate, such analyses can then become intractable. In this work, we present a fast and precise parameter estimation method using relative binning. We extend the method so that it can be used to analyze individual GW as well as two overlapping GW signals in the 3G-era. We adapt our method to perform strong lensing searches. Specifically, we analyze
\begin{itemize}
    \item A large set of simulated BBH signals to establish validity. 
    \item Real LIGO-Virgo events GW190412~\cite{LIGOScientific:2020stg} and GW190814~\cite{LIGOScientific:2020zkf}.
    \item A set of simulated individual as well as overlapping GW signals of the 3G-era.
    \item The event pair GW191103-GW191105 as well as a set of simulated pair, to investigate strong lensing hypothesis
\end{itemize}
