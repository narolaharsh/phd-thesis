...
\begin{equation}
\ln \mathcal{L}(\vec{\theta}) = -\frac{1}{2}\left<\vec{d}-\vec{h}_1(\vec{\theta}) - \vec{h}_2(\vec{\theta}) |\vec{d}-\vec{h}_1(\vec{\theta}) - \vec{h}_2(\vec{\theta})\right>\,,
\end{equation}
where $\vec{\theta}$ now represent a 30 dimensional parameter space in place of 15 dimensional. Follwing the steps from Eq.~\eqref{eqn:indi-likelihood}-Eq.~\eqref{eqn:short-l}, we can factorise the likelihood into
\begin{align}
    ...
    & = \ln \mathcal{L}(\vec{\theta}) = \ln \mathcal{L}_{dh_1} - \frac{1}{2} \ln \mathcal{L}_{h_1h_1} + \ln \mathcal{L}_{dh_2} - \frac{1}{2} \ln \mathcal{L}_{h_2h_2} \,.
\end{align}
