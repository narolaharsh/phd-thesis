...
Figure~\ref{fig:dxp-ratio} with the top right panel of Figure~\ref{dxp-ratio}. 
...
\textit{summary data} based only on the reference waveform and useful for the 
computation of the inner products in Eq.~\eqref{eqn:short-l}~\cite{Leslie:2021ssu}. These are:
\begin{eqnarray}
...
which together with Eq.~\eqref{eqn:short-l} enables us to approximate the likelihood. Provided the number of bins is reduced compared to the exact method, this approach should offer a rapid likelihood evaluation, leading to a rapid parameter inference.
...
\begin{figure}[h]
    ...
    \caption{JS divergence values to quantify if the measurements made using relative binning method are consistent with the ones mades using exact computation. The black line marks the JS divergence below which two measurements are indistinguishable. The plot contains the JS divergence values for all parameters for all signals, simulated as well as real, analyzed in this chapter. For compactness, we use the same marker for the set of 3G signals (circles) and for the set of lensed signals (crosses). We uses different coloured markers for LIGO-Virgo events. All JS divergence values remain below the threshold, indicating that measurements obtained using relative binning method are reliable.}
    \label{fig:master-jsd}
\end{figure}

To quantify the mismatch between the posterior samples obtained using the relative binning method to the ones obtained with the exact method, we use the Jensen-Shannon (JS) divergence~\cite{2020SciPy-NMeth}. 
We do not perform a exact parameter estimation run due to computation cost. Instead, we reweigh the posterior samples obtained using the relative binning 
...
\label{reweight}
We reweigh the posterior samples obtained using relative binning to match the samples obtained with the exact likelihood to test the method's efficiency. 
...
samples by $p^{\text{RB}}(\vec{\theta})$. To perform the reweighting, for each posterior sample $\vec{\theta}$, we compute the likelihood according to the exact method, which we denote by $\mathcal{L}^{\text{X}}(\vec{\theta})$. 
Using $\mathcal{L}^{\text{RB}}(\vec{\theta})$, $\mathcal{L}^{\text{X}}(\vec{\theta})$, 
we reweigh $p^{\text{RB}}(\vec{\theta})$ to obtain the probability 
for $\vec{\theta}$ according to the exact method:
\begin{equation}
...
GW200112\_155838 & 78.10 & 1103.53 \\
 ...
 GW200220\_061928 & 282.50 & 224.91 \\
    ...
    \end{tabular}
    \caption{Table of events analysed in context of 3G detectors. The second column show the maximum likelihood values of the total mass obtained from public data release of LVK. The third column shows the respective network SNRs if they were observed by a network of 3G detectors. The events were detected by LIGO-Virgo interferometers during their third observation runs and the naming convention follows the pattern GWYYMMDD\_HHMMSS. }
...
This process is repeated for each of the six events and the results are shown in Figure~\ref{fig:master-jsd}. For compactness, we use circle maker for all events since our aim here is to verify if the JS divergence is below the threshold. Here too the JS divergence for each parameter remains well below the threshold, indicating that the posterior distributions produced by the relative binning method and the 
exact methods are statistically indistinguishable.
...
\begin{figure}
    ...
    \caption{Analysing the pair, GW191103 (red) and GW191105 (green), under strong lensing hypothesis (blue) using the relative binning method. Left plot shows the measurements of the common parameter between the two events and the right plot shows the measurements of lensing parameters. The event pair was of great interest for strong lensing searches due to significant overlaps in the posterior distributions of individual events, though the analyses turned out to be inconclusive.}
...
The left plot of Figure~\ref{fig:lensed_pair} shows a truncated corner plot of the common parameters between the two images, while the right one presents the corner plot of the relative lensing parameters connecting the lensing parameters of images 1 and 2.
To quantify the accuracy of our results, we follow the same reweighting steps mentioned towards the end of Section~\ref{reweight} in order to calculate the JS divergence between the relative binning samples and the exact samples for each parameter. 
The JS divergences are marked with hexagon marker in Figure~\ref{fig:master-jsd}, which are all below the threshold. 

Finally, we report that the log of coherence ratio -- a statistic to quantify the overlap between the posterior distribution of two images -- obtained for GW191103--GW191105 is 4.5~\cite{Janquart:2023osz, Lo:2021nae, Janquart:2021qov}. This is in favor of the lensing hypothesis and comparable to the one reported by the lensing follow-up searches~\cite{Janquart:2023mvf}. 
...
Here we use the comprehensive parameter estimation analyses performed in previous sections to asses the speed-up obtained by the relative binning compared to exact method.
Figure~\ref{fig:speed} shows the speed-up achieved by the relative binning method over the exact method as a function of the total mass of the systems. The colorbar shows the ratio of the length of the uniform frequency grid to the length of the corresponding RBGrid. We denote the ratio by $\Delta$. For the instances where we performed parameter estimation runs with exact likelihood and relative binning approximation, we report the ratio of the run time of the two. For the instances where we only performed relative binning runs, we show the ratio of the average single likelihood evaluation time taken between the two methods calculated over a set of $10^4$ points. The latter is an approximation but makes it possible to 
...
this effect is partially visible in Figure~\ref{fig:speed}. However, the 3G injections considered for the analysis have a considerably higher SNR than the corresponding 
...
%by the relative binning method is below 1 (see Figure~\ref{fig:speed}). 
%This can be explained by the large network SNR ($\simeq 100$) of this particular event. 

Joint parameter estimation reports a similar speed-up as individual parameter estimation. Here we note that the exact joint parameter estimation framework used 
...
In this chapter, we have presented a relative binning based framework to perform fast parameter estimation. We have shown the robustness of the method by performing parameter estimation on a large set of simulated as well as real events. We have also extended the method to perform parameter estimation on overlapping signals and to perform strong lensing searches. This marks the fist instance where the relative binning method has been tested in a comprehensive manner. Our implementation of the bin selection and relative binning parameter estimation framework can be found in the git repository at~\cite{janquart:2022}.

The performance of our method depends on the choice of the fiducial waveform. In this work, we have assumed that the fiducial waveform is either (a) the same as the injected waveform for simulations, or (b) the maximum likelihood waveform for real events. However, the method can be seeded in a more realistic way by using the best-fitting template reported by the templated-based searches or maximum likelihood estimator routines can be used to determine the fiducial waveforms' parameters. We plan to work on the choice of the fiducial waveform and understanding how it can impact the performance of our method in the future. This is an important step if one wants to use relative binning techniques on real data, where we do not know parameters in advance.

While the relative binning method is shown to work well in the joint parameter estimation of the overlapping signals, it is crucial to determine when such a joint parameter estimation is required, i.e., if the signals are sufficiently apart, they can be analysed separately which may help reudce the cost of the analysis. This question was addressed in the in-depth analysis performed by Baka et al [in preparation]. The codebase presented in this chapter served as a building block for it. 
