...
When performing the Bayesian parameter inference on a GW signal, we need to simulate a large number ($\approx 10^6~\mathrm{to}~10^7$) of waveforms. This is the most expensive part of the process. Each waveform is generated on a uniform frequency grid whose minimum and maximum frequencies depend on the bandwidth (typically 20 Hz to 2048 Hz) of the GW detectors. The resolution of the frequency grid is set to $1/T$, where $T$ is the duration of the signal. Generating a large number of waveforms on a uniform grid increases the computational cost. The relative binning method seeks to address this issue by generating the waveform on a sparse frequency grid and interpolating on the remaining points in such a way that the accuracy of the analysis has a negligible effect. The usage of the sparse frequency grid leads to a speed up. 
...
(also known as the $L$-frame); and the indices $l$ and $m$ label the different modes. The $L$-frame is the frame whose $z$-axis is chosen along the orbital angular momentum of the system. The coefficients $C^{+}_{l, m}$ and $C^{\times}_{l, m}$ account for 
...
\begin{figure}[h]
    ...
    \caption[A schema of two gravitational wave signals overlapping]{An illustration of two gravitational wave signals (blue and orange) arriving at the Earth at nearly the same time. At the Earth based GW detector, we can only measure the sum of the two (black line).}
...
\textit{Jensen-Shannon divergence:} Once we have the posterior samples from the exact method (either by performing exact analysis or by reweighting), we quantify the similarities between the two using the Jensen-Shannon (JS) divergence~\cite{JSD:61115}. The JS divergence yields a finite value between 0nat and 1nat. A value closer to 0nat corresponds to greater similarity between two distributions. If we denote the two distributions we want to compare by $p$ and $q$, then the JS divergence is given by
\begin{equation}
...
\begin{figure}[h]
    ...
    \caption[Parameter estimation results for the signal GW190412]{The parameter measurements obtained using the relative binning framework (blue curves) are consistent with the ones reproduced from the public data release of the LVK collaboration (grey) for GW190412.}
    \label{fig:gw190412}
\end{figure}
...
\begin{figure}[h]
    \centering
    \includegraphics[width=.75\textwidth]{../figures/overlap_production_gw190814_corr_branch_gwtc.pdf}
    \caption[Parameter estimation results for the signal GW190814]{The parameter measurements obtained using the relative binning framework (blue curves) are consistent with the ones reproduced from the public data release of the LVK collaboration (grey curves) for GW190814.}
    \label{fig:gw190814}
\end{figure}
...
\subsection{Validation on the parameter estimation of 3G-era events}
...
\begin{figure}
    \centering
    \includegraphics[width=.6\textwidth]{../figures/overlapping_signals_A.pdf}\vspace{.25cm}
    \includegraphics[width=.6\textwidth]{../figures/overlapping_signals_B.pdf}
    \caption[Joint parameter estimation on two overlapping gravitational wave signals]{Comparison of parameter estimation results when two signals are in isolation and when two signals are overlapping.}
    % Measurements in isolation establish the benchmark for accuracy. The agreement between the overlapping signal analysis and isolated signals analysis indicates that the relative binning framework to perform joint parameter estimation can be extended to a wider parameter space.}
    \label{fig:overlapping-signal}
\end{figure}
We collectively present the results for overlapping signals analysis in Figure~\ref{fig:overlapping-signal}. Let us refer to the two signals as signal A and signal B. The red (green) lines in the top (bottom) plot show how accurately we can recover signal A (signal B) when they are in isolation. This establishes the benchmark for accuracy. The black lines in the plot show the injected value of the parameters. For this example, we chose two signals with chirp mass of around 22 $M_{\odot}$ and 47 $M_{\odot}$. The injected value of the remaining parameters can be gauged from the black lines. The blue lines in both plots show the results of joint parameter estimation using the relative binning framework, i.e., how well the parameters of the two signals are recovered when they are overlapping. The agreement of the blue curves with the green and red curves suggests that our relative binning framework can be reliably extended to perform joint parameter estimation when two (or more) GW signals are overlapping. 

\subsection{Validation on the strongly lensed events}
...
Besides the JS divergence, we could also check for the overlap between the joint posterior distribution of GW91103--GW191105 obtained using different methods. Specifically, we can compare the joint posterior distribution obtained using the relative binning method and the exact method. The overlap is quantified using the coherence ratio which is defined as~~\cite{Lo:2021nae, Janquart:2021qov}
\begin{equation}
     \mathcal{C}^{\mathrm{Lensed}}_{\mathrm{Unlensed}} = \frac{p(\vec{d}_1, \vec{d}_2 | \vec{\theta}, \vec{\theta}_L)}{p(\vec{d}_1 | \vec{\theta}_1) p(\vec{d}_2 | \vec{\theta}_2)}.
\end{equation}
Namely, it is the ratio of evidence (Eq.~\ref{eq:evidence}) when the data from two images ($\vec{d}_1, \vec{d}_2$) are jointly analysed under lensed hypothesis to evidence when each event is analysed separately under the unlensed hypothesis. Finally, we report that the (log) of coherence ratio obtained for the GW191103--GW191105 pair is 4.5~\cite{Janquart:2023osz, Lo:2021nae, Janquart:2021qov}. The value is comparable to the one reported by the lensing follow-up searches~\cite{Janquart:2023mvf}. Despite the high degree of overlap, the investigations on the pair being strongly lensed remained inclusive due to a large false alarm rate~\cite{Janquart:2023mvf}. 
