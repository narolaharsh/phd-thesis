...
When two or more signals merge close to each other, we refer to them as overlapping signals (top panel of Figure~\ref{fig:lensing-overlaps}). It is intuitive to think that if two signals are far apart, we can analyze each of them while ignoring the presence of the other signal. However, when they are close, they need to be analysed jointly to avoid biases in the measurements. A comprehensive investigation on this topic was done by Tomek et al [in preparation]. Tomek et al extend the relative binning method developed in this work to perform \textit{joint} parameter estimation on two GW signals. 
...
\iffalse
The LIGO-Virgo interferometers have not yet observed any overlapping signals and it is unlikely that we do so at current sensitivities. Therefore, the state-of-the-art parameter estimation pipelines are designed to analyze a single, isolated signal. However, in the 3G era, due to the increased detection rate and increased duration of GW signals, we may often encounter overlapping GW signals. Therefore, it is crucial to develop parameter estimation methods to analyze overlapping signals. 
\fi
...
Figures~\ref{fig:gw190412} and \ref{fig:gw190814} show a qualitative agreement between the posterior distributions obtained by the regular method (without making any approximation) and the relative binning method. Similarly, we analyzed GW191103 and GW191105 with the relative binning method, which led to a comparable agreement with the regular method. 
...
\subsection{On simulated signals for 3G interferometers}

To further test our method, we analyze a subset of LIGO-Virgo events as they would be appear in 3G interferometers. 
...
We analyze eight simulated lensed pairs and the GW191103--GW191105 event pair to test the relative binning framework for joint parameter estimation. 
