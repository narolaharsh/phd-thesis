...
\subsubsection{Checking the efficiency: Reweighing of the relative binning posterior samples}
\label{reweight}
...
samples by $p^{\text{RB}}(\vec{\theta})$. To perform the reweighed, for each posterior sample $\vec{\theta}$, we compute the likelihood according to the usual method, which we denote by $\mathcal{L}^{\text{RG}}(\vec{\theta})$. 
Using $\mathcal{L}^{\text{RB}}(\vec{\theta})$, $\mathcal{L}^{\text{RG}}(\vec{\theta})$, 
we can reweigh $p^{\text{RB}}(\vec{\theta})$ to obtain the probability 
...
To test our method, we analyze a subset of GWTC-3 events as they would be seen by these detectors. 
We arbitrarily choose six events with varying total masses from the GWTC-3 catalogue, specified in table~\ref{tab:gwtc-3-table}. 
...
A representative, truncated corner plot for this analysis is shown in Figure~\ref{3g-event} for the GW200220\_061928 event. 
...
We again reweigh the posterior samples obtained using the relative binning method following the procedure in Appendix~\ref{reweight} and 
calculate the JS divergence between the reweighed samples and the relative binning samples. 
This process is repeated for each of the six events and the results are shown in Figure~\ref{three-g-js}. 
