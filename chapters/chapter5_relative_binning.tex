...
Note that the $\vec{d}$ is now replaced by $\vec{d}_1$ and $\vec{d}_2$ at relevant places. Next, we construct the RBGrid only for image 1. Compute image 2 waveform using the Eq.~\eqref{eqn:wf-mapping}. We use the RBGrid to generate the summary data and then, using the summary data, we can approximate the likelihood. 
