...
############# injecting the offset glitch ###############################

offset_white_signal_noise_glitch_timeseries, offset_glitch_start_time = utils.add_glitch(glitch = glitch, noise = white_signal_noise_timeseries, t_inj = args.glitch_trigger_time - args.signal_delta_t - 0.4, time_array = ifos[0].time_array)

offset_coloured_signal_noise_glitch_timeseries = utils.white_timeseries_to_coloured_timeseries(offset_white_signal_noise_glitch_timeseries, asd, sampling_frequency, args.minimum_frequency, duration=args.duration)


############### saving glitch only data #####################

if plot_tcp:
    fig, axes = plt.subplots(1, 2, figsize = (8, 4), sharey = True, sharex = True, gridspec_kw = {'wspace': 0.075})
    ...
    for ii in range(2):
        if ii==0:
            ...
            timeseries_to_plot = TimeSeries(offset_coloured_signal_noise_glitch_timeseries, times = ifos[0].time_array, channel = f'{ifo.name}:STRAIN')
    ...
    fig.savefig(f'{args.outdir}/{args.label}_offset.png', bbox_inches = 'tight', dpi = 200)
    fig.savefig(f'../../figures/{args.label}_offset.png', bbox_inches = 'tight', dpi = 200)
